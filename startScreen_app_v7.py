...
TURN_COUNT_ENEMY_BOSS = settings["TURN_COUNT_ENEMY_BOSS"]
...
def generate_dungeon(rows, cols, isBoss = False):
    ...
    return dungeon, all_keys, all_mana, all_healing, boss_placed, boss_position, original_tile, boss_health, all_poison  # Return dungeon, all keys and mana tiles
...
# Handle tile effects
def handle_tile(dungeon, all_mana, all_keys, all_healing, boss_placed, boss_health, all_poison):
    ...
    if [r, c] != previous_position:
       ...
       # print(f"You stepped on a {tile_type} tile.")
        ...
        if tile_type == "Enemy":
            boss_health_tpl = combat(False, boss_health)
            boss_health = boss_health_tpl[0]
            enemy_kill = boss_health_tpl[1]
            if enemy_kill:
            ...
            boss_health_tpl = combat(True, boss_health)  # Engage in combat with the boss if player lands on it
            boss_health = boss_health_tpl[0]
            if boss_health <= 0:
...
def combat(isBoss, boss_health):
    ...
    return boss_health, enemy_kill  # Return the updated boss health
...
def move_player(direction, dungeon, all_keys, all_mana, all_healing, boss_placed, boss_health, all_poison):
    r, c = player["position"]
    ...
    if moved:
        boss_health = handle_tile(dungeon, all_mana, all_keys, all_healing, boss_placed, boss_health, all_poison)
        # Manage poison tile lifespan
        player["moves_made"] += 1
        player["turns_taken"] += 1  # Increase turn count
        if player["moves_made"] % 10 == 0:  # Poison starts after 10 moves
            spawn_poison_tiles(dungeon, all_poison)
        if player["poisoned"] and player["moves_made"] % 15 == 0:  # Spawn Elixir tiles every 10 moves
            spawn_elixir_tiles(dungeon)
        #handle_poison_tiles(dungeon, all_poison)  # Call with the all_poison list
        # Update keys' and mana tiles' lifespan
        update_lifespan(dungeon, all_keys, all_mana, all_poison)    
        # Track player moves and spawn enemies after every 3 moves

    if not boss_placed and player["moves_made"] % TURN_COUNT_ENEMY == 0:
        spawn_enemy(dungeon)
    if boss_placed and player["moves_made"] % TURN_COUNT_ENEMY_BOSS == 0:
        spawn_enemy(dungeon)
    if not boss_placed and player["turns_taken"] % TURN_COUNT_KEY_SPAWN == 0:
...
def move_boss(dungeon, player_position, boss_position, original_tile, boss_health):
    ...
    if (br, bc) == (pr, pc):
        ...
        boss_health_tpl = combat(True, boss_health)  # Adjust health if needed
        boss_health = boss_health_tpl[0]
...
def update_lifespan(dungeon, all_keys, all_mana, all_poison):
    ...
    # Handle poison tiles' lifespan
    for poison in all_poison[:]:
        poison["lifespan"] += 1
        if poison["lifespan"] >= 12:  # Remove poison tiles after 12 turns
            pr, pc = poison["position"]
            dungeon[pr][pc] = "Empty"
            all_poison.remove(poison)
            print(f"Poison tile at ({pr}, {pc}) has expired and been removed.")
...
def spawn_poison_tiles(dungeon, all_poison):
    empty_tiles = [(r, c) for r in range(GRID_ROWS) for c in range(GRID_COLS) if dungeon[r][c] == "Empty"]
    if empty_tiles:
        r, c = random.choice(empty_tiles)
        ...
        if dungeon[r][c] != "Portal":  # Ensure we are not accidentally adding a portal
            dungeon[r][c] = "Poison"
            all_poison.append({"position": (r, c), "lifespan": 0})
...
def check_win(dungeon, boss_health, isBoss):
    if isBoss:
        if boss_health <= 0:
...
def handle_poison_tiles(dungeon, all_poison):
    for poison in all_poison[:]:  # Iterate over a copy of the list
        poison["lifespan"] += 1  # Increase the lifespan each turn
        if poison["lifespan"] >= 12:  # Remove poison tiles after 12 turns
            r, c = poison["position"]
            dungeon[r][c] = "Empty"  # Set the tile to "Empty"
            all_poison.remove(poison)  # Remove poison from the list
            print(f"Poison tile at ({r}, {c}) has expired and is removed.")

# Main game loop
def main(settings):
    global GRID_ROWS, GRID_COLS, KEY_START, TRAP_START, PORTAL_START, HEALING_START, MANA_START, ENEMY_START, MANA_KILL, TRAP_DAMAGE, MAX_HEALTH, TURN_COUNT_ENEMY, TURN_COUNT_ENEMY_BOSS, TURN_COUNT_KEY_SPAWN, TURN_COUNT_MANA_SPAWN, TURN_COUNT_KEY_DEATH, TURN_COUNT_MANA_DEATH, WIN_CONDITION_KEY
    ...
    dungeon, all_keys, all_mana, all_healing, boss_placed, boss_position, original_tile, boss_health, all_poison = generate_dungeon(GRID_ROWS, GRID_COLS, isBoss=False)  # Generate new dungeon with Boss tile
    ...
    while running and player["health"] > 0:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    moved, boss_health = move_player("up", dungeon, all_keys, all_mana, all_healing, boss_placed, boss_health, all_poison)
                elif event.key == pygame.K_DOWN:
                    moved, boss_health = move_player("down", dungeon, all_keys, all_mana, all_healing, boss_placed, boss_health, all_poison)
                elif event.key == pygame.K_LEFT:
                    moved, boss_health = move_player("left", dungeon, all_keys, all_mana, all_healing, boss_placed, boss_health, all_poison)
                elif event.key == pygame.K_RIGHT:
                    moved, boss_health = move_player("right", dungeon, all_keys, all_mana, all_healing, boss_placed, boss_health, all_poison)
        ...
        boss_health = handle_tile(dungeon, all_mana, all_keys, all_healing, boss_placed, boss_health, all_poison)  # Regular tile interactions
                # Only handle healing tiles if the player moved
        # Manage poison tile lifespan
        #handle_poison_tiles(dungeon, all_poison)
        ...
        if boss_placed and moved:
            # **Proximity Check: Trigger combat if player is in any of the 8 surrounding tiles**
            br, bc = boss_position  # Boss's current position
            pr, pc = player["position"]  # Player's position

            # List of 8 adjacent tiles around the boss (including diagonals)
            adjacent_tiles = [
                (br - 1, bc), (br + 1, bc), (br, bc - 1), (br, bc + 1),  # Vertical and horizontal
                (br - 1, bc - 1), (br - 1, bc + 1), (br + 1, bc - 1), (br + 1, bc + 1)  # Diagonal
            ]
            # Ensure the tiles are within the grid bounds
            valid_adjacent_tiles = [
                (r, c) for r, c in adjacent_tiles if 0 <= r < GRID_ROWS and 0 <= c < GRID_COLS
            ]        
            # Check if the player's position is in any of the 8 surrounding tiles
            if (pr, pc) in valid_adjacent_tiles:
                print("The Boss has encountered the player in AoE!")
                # Trigger combat between boss and player
                boss_health_tpl = combat(True, boss_health)  # Adjust health if needed     
                boss_health = boss_health_tpl[0] 

        # Check win condition
        if check_win(dungeon, boss_health, isBoss):
            screen.fill((0, 0, 0))  # Clear the screen
            pygame.display.update()
            ...
            dungeon, all_keys, all_mana, all_healing, boss_placed, boss_position, original_tile, boss_health, all_poison = generate_dungeon(GRID_ROWS, GRID_COLS, isBoss=True)
            isBoss = True
