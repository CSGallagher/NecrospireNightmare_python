...
# List to store healing tiles with their position, inactive turns, and active status
all_healing = []
...
def generate_dungeon(rows, cols):
    ...
    return dungeon, all_keys, all_mana, all_healing  # Return dungeon, all keys and mana tiles
...
def handle_healing_tiles(dungeon, all_healing):
    for healing_tile in all_healing:
        ...
        # Debug: print the current state of the healing tile
        print(f"Checking healing tile at {r}, {c}: Active = {healing_tile['active']}, Inactive Turns = {healing_tile['inactive_turns']}") # Debugging
        ...
        if healing_tile["active"]:
            ...
            # Debug: state transition when the tile is active
            print(f"Tile at {r}, {c} is active, reset inactive turns.") # Debugging

        else:
            # Only increase inactive turns if the tile is inactive
            healing_tile["inactive_turns"] += 1  # Increase inactive turns if inactive
            # Debug: state transition when the tile is inactive
            print(f"Tile at {r}, {c} is inactive, inactive turns = {healing_tile['inactive_turns']}") # Debugging
            ...
            if healing_tile["inactive_turns"] >= 10:
                ...
                # Debug: state transition when the tile respawns
                print(f"Tile at {r}, {c} has been inactive for 10 turns. Respawning healing tile.") # Debugging

        # If the healing tile is inactive, it should be marked as empty
        if not healing_tile["active"]:
            dungeon[r][c] = "Empty"  # Mark as empty when inactive
                        # Debug: mark tile as empty when inactive
            print(f"Tile at {r}, {c} is inactive. Marking as Empty.") # Debugging
...
def main(settings):
    ...
    dungeon, all_keys, all_mana, allhealing = generate_dungeon(GRID_ROWS, GRID_COLS)  # Now also returning all keys and mana_tiles
    ...
    while running and player["health"] > 0:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    moved = move_player("up", dungeon, all_keys, all_mana)
                elif event.key == pygame.K_DOWN:
                    moved = move_player("down", dungeon, all_keys, all_mana)
                elif event.key == pygame.K_LEFT:
                    moved = move_player("left", dungeon, all_keys, all_mana)
                elif event.key == pygame.K_RIGHT:
                    moved = move_player("right", dungeon, all_keys, all_mana)
        ...
        handle_tile(dungeon, all_mana, all_keys)  # Regular tile interactions
